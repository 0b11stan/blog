...
typiquement `site:target.net admin` peut souvent apporter plein d'infos
intéressantes
...
de la ligne évite permet de jouer cette commande en background et donc de
redonner la main au shell.

## Attaque par dictionnaire

Qui à besoin de burp intruder quand il y à [ffuf](https://github.com/ffuf/ffuf)

Et d'un bon dictionnaire, aujourd'hui il peuvent s'installer avec le paquet
[seclists](https://github.com/danielmiessler/SecLists) sur la majorité des OS
orienté offensif.

Par exemple pour attaquer le champ password d'un POST http.
...
on peut utiliser plusieurs wordlists avec des keyword donnés (ici `W1` et `W2`)
```bash
...
c'est quand on appele directement une route qui devrait être protégé par une
authentification mais qui ne l'est pas

par exemple quand il y à une liste d'item et qu'on peut accéder aux détail sous
la route `/items/<number>`, on peut tester de butforce `<number>`.

## File upload

Si il y à une vérification sur l'extention du fichier, on peut ajouter un null
byte et donc envoyer un fichier avec ce nom : `upload.php%00.png`

quand on appelera ce fichier, il sera traité par le serveur comme un fichier
php

## LFI

Tester [cette page de l'owasp](https://owasp.org/www-project-web-security-testing-guide/v42/4-Web_Application_Security_Testing/07-Input_Validation_Testing/11.1-Testing_for_Local_File_Inclusion)
...
le problème c'est que les fichiers php sont executés par le serveur quand on
les appels mais en fait en utilisant un filter c'est possible d'extraire le
code du fichier sans qu'il s'execute

On peut extraire le code un fichier en utilisant un filtre
...
mini sqlmap écrit en python en moins de 100 lignes de codes mais qui fait tout
ce qu'il faut.

champs utiles à extraires : 
...
a la fin de [la room sqlibasics](https://tryhackme.com/room/sqlibasics) sur tryhackme
il y à plein d'idée de trucs pour aller plus loin
...
Ca veux dire qu'on à une erreur en retour qui nous donne plein d'infos sur les
raisons qui ont cassé l'app, c'est le plus simple, il faut juste jouer avec
l'input jusqu'a ce qu'on construise une requette qui passe.

### Boolean based

permet sur tout de trouver des nom de db ou de table

On à beaucoup moins d'infos aucune erreur n'est retourner. 

1. identifier un cas nominal (payload qui marche normalement)
2. essayer de faire une expression SQL simple : marche plus
...
on peut utiliser du coup des `>` et `<` pour trouver les lettre en un minimum de
requettes ce qui est cool, il suffit de faire un pas à pas pour trouver les
bonnes lettre (sqlmap le fait aussi très bien mais on peut peut être faire mieux)

TODO : scriper en python
...
1. d'abord il faut chercher à casser le truc
2. comprend ce qui à casser
3. LIRE LES MESSAGES D'ERREUR
4. définir les tables qu'on vise
5. définir le nb de champs avec des null

pas oublier d'encoder les requettes en url
...
La variable `register_globals` [doc](https://www.php.net/manual/ro/security.globals.php),
si elle est mise sur "on" dans le fichier de config de php, permet de surcharger
des variables lors des appels. Par exemple le payload `_SESSION[logged]=1` en
argument d'un GET va set la variable `$_SESSION["logged"]` à `1` quand le code
php sera executé.

`assert(` en php5 peut prendre une string en parametre, ça peut permettre de
faire executer du code à la volée https://www.php.net/manual/en/function.assert.php
...
Si jamais la cible n'a pas fetch mais qu'il faut faire un POST depuis son 
navigateur, on peut étendre le payload dernier:
```html
<script>
  var urlTarget = "...";
...
is a direct download, and here is the php code without the license :
```php
<?php
set_time_limit (0);
